...
const DatosAlbum = () => {
  ...
  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem("Tracks-favoritos")) || [];
    setFavoritos(new Set(stored));
  }, []);

  const toggleFavorito = (idTrack) => {
    const nuevoSet = new Set(favoritos);
    if (nuevoSet.has(idTrack)) {
      nuevoSet.delete(idTrack);
    } else {
      nuevoSet.add(idTrack);
    }
    setFavoritos(nuevoSet);
    localStorage.setItem("Tracks-favoritos", JSON.stringify(Array.from(nuevoSet)));
  };

  const esFavorito = (idTrack) => favoritos.has(idTrack);
